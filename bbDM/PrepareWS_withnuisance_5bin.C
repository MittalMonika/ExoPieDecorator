...
void createRegion(RooRealVar met, TH1F* h_sr_bkg , TH1F* h_cr_bkg, 
  ...
  ralbc_sr_bkg.add(rrvbc_sr_bkg[4]);
  ...
  */
  ...
  std::cout<<" ratio "<< htf_cr_bkg->GetBinContent(1)
	   ...
	   <<" "<<htf_cr_bkg->GetBinContent(4)
    	   <<" "<<htf_cr_bkg->GetBinContent(5)
	   <<std::endl
	   <<"  SR yield 1 =" <<h_sr_bkg->GetBinContent(1)
	   <<" 2 "<<h_sr_bkg->GetBinContent(2)
	   <<" 3 "<<h_sr_bkg->GetBinContent(3)
	   <<" 4 "<<h_sr_bkg->GetBinContent(4)
	   <<" 5 "<<h_sr_bkg->GetBinContent(5)
	   <<std::endl
	   <<" "<<h_cr_bkg->GetBinContent(1)
	   <<" "<<h_cr_bkg->GetBinContent(2)
	   <<" "<<h_cr_bkg->GetBinContent(3)
	   <<" "<<h_cr_bkg->GetBinContent(4)
    	   <<" "<<h_cr_bkg->GetBinContent(5)
	   <<std::endl   ;
  ...
  RooRealVar tf5 ("tf5_"+region_proc_cr+anacat_,"tf5_"+region_proc_cr+anacat_,bincontents_htf_cr_bkg[4]) ;
  ...
  */
  ...
  tf_stats_err_vector.push_back(htf_cr_bkg->GetBinError(5) / htf_cr_bkg->GetBinContent(5) ) ;
  ...
  // possible bug above??? Fixme??
  
  std::vector<std::string> rfv_tf_stats_err_vector = createnuisance(tf_stats_err_vector, 5, syst_counter++);
  ...
  rfv_bin5  += rfv_tf_stats_err_vector[4] ;
  ...
  RooRealVar rrv_stats_err_bin5("rrv_stats_err_"+region_proc_cr+anacat_+"_bin5", "rrv_stats_err_"+region_proc_cr+"_bin5",tf_stats_err_vector[4],0,2.*tf_stats_err_vector[4]);
  ...
  ral_bin5.add(rrv_stats_err_bin5);
  ...
  for (int isys=0; isys < (int) nuisIndex.size(); isys++){
    std::vector<std::string> add_logN_systematic = createnuisance(nuisanceValue[nuisIndex[isys]], 5, syst_counter++);
    ...
    rfv_bin5 += "*"+add_logN_systematic[4];
    ...
    std::cout<<" bin 5 stats unc "<< rfv_bin5<<" after including "<<nuisanceName[nuisIndex[isys]]<<std::endl;
    ...
    ral_bin5.add(*rrv_syst); // it has to be pointer for some reason, otherwise it gives seg fault, not sure yet, why? 
  ...
  */
  ...
  RooFormulaVar rfv_cr_bkg5("rfv_"+region_proc_cr+"5"+anacat_,"Background yield in control region, bin 5","@0*@1",RooArgList(TF5,rrvbc_sr_bkg.at(4)));
...
void PrepareWS_withnuisance_5bin(TString model_="monoHbb",TString analysiscategory_="merged", TString mode__ = "RECREATE", TString inputdir=".", TString inputfile="AllMETHistos.root", TString year="2016"){
  ...
  //Double_t bins[]={200,240, 300, 400, 550, 1000}; // v1
  //Double_t bins[]={200,235,290,400,550,1000}; //v2
  Double_t bins[]={200,230,290,410,550,1000}; //v3
