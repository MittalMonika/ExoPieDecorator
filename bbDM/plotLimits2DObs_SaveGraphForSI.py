...
def makePlot2D(filepath,foutname,medcfg,chicfg,header='',offshell=False):
  ...
  rootfilename = filepath.replace(".txt",".root")
  graphroot = TFile(rootfilename,"RECREATE")
  graphroot.cd()
  hs['exp'].SetLineColor(2)
  hs['exp'].SetLineWidth(3)
  hs['exp'].GetXaxis().SetTitle("m_{a} [GeV]")
  hs['exp'].GetXaxis().SetTitle("tan#{beta}")
  hs['exp'].SetLineColor(2)
  hs['exp'].SetName("expected_curve")
  ...
  hs['expup'].Write()
  hs['expdown'].Write()
  hs['obsclone'].SetName("observed_curve")
...
#  hs['obsup'].Draw('CONT3 SAME')
  ...
  ctemp = root.TCanvas()
  hs['exp'].Draw('contlist')
  ctemp.Update()
  objsexp = root.gROOT.GetListOfSpecials().FindObject('contours')
  saveexp = root.TGraph((objsexp.At(0)).First())
  ...
  fsave.WriteTObject(saveobs,'observed_curve')
  
  saveexp.SetLineColor(2)
  saveexp.SetLineWidth(3)
  saveexp.GetXaxis().SetTitle("m_{a} [GeV]")
  saveexp.GetYaxis().SetTitle("tan#beta")
  fsave.WriteTObject(saveexp,'expected_curve')
  fsave.Close()
...
makePlot2D('limits_tanb_vs_ma_scan.txt','SIGraphs/graphs',(200,10,2000),(102,1, 700),'Z`-B',True)
