...
parser.add_argument("-d", "--dataset", dest="dataset", default="AllMETHistos_monohbb_v12_07_09_updatedPhoVeto.root")
...
dataset_RP =args.dataset
year     = args.year
category = args.category
regions  = args.reg
modelName = args.model

print("performing mAscan",modelName)
mAscan=True
zp2hdm_mzpscan =False
zp2hdm_fullscan =True
zpb_mzpscan=False
zpb_fullscan=True
if category=='R':
...
year_="_"+year
def getEndPart_RP(reg):
    end_=''
    if reg=='SR':
        end_='''
ratett_'''+category+year_+''' rateParam SR tt 1. [0.5,1.5]        
ratezjets_'''+category+year_+''' rateParam SR zjets 1. [0.5,1.5]
* autoMCStats 10 0 1
'''
    if reg =='TOPE':
        end_='''  
ratett_'''+category+year_+''' rateParam TOPE tt 1. [0.5,1.5]
* autoMCStats 10 0 1
'''
    if reg =='TOPMU':
        end_='''  
ratett_'''+category+year_+'''  rateParam TOPMU tt 1. [0.5,1.5]
* autoMCStats 10 0 1
'''
    if reg =='ZMUMU':
        end_='''    
ratezjets_'''+category+year_+'''  rateParam ZMUMU dyjets 1. [0.5,1.5]        
* autoMCStats 10 0 1
'''
    if reg =='ZEE':
        end_=''' 
ratezjets_'''+category+year_+'''  rateParam ZEE dyjets 1. [0.5,1.5]        
* autoMCStats 10 0 1
'''       
    if end_ =='' :
...
def getUpperPart2(reg,cat):
	top_= 'shapes * '+reg+' monoHbb_'+year+'_WS.root ws_monoHbb_'+cat+'_'+year+':monoHbb'+year+'_'+cat+'_'+reg+'_$PROCESS ws_monoHbb_'+cat+'_'+year+':monoHbb'+year+'_'+cat+'_'+reg+'_$PROCESS_$SYSTEMATIC'+'\n'
        addst_ =''
#monika rateparam 
#1. remove rph lines
#2. 
...
def getUpperPart2_RP(reg,cat):
    top_= 'shapes * '+reg+'   '+dataset_RP+'  monoHbb'+year+'_'+cat+'_'+reg+'_$PROCESS monoHbb'+year+'_'+cat+'_'+reg+'_$PROCESS_$SYSTEMATIC'
    return top_
...
def getSignalHists(doc,model):
	...
	samples  = []
	if model=="2hdma":
                print("I am in 2hdma model", model)
		...
		mzp_list  = parameters[1]['mzp']
                ...
                #print("I am in zphdma model", model)
		...
		mzp_list  = parameters[1]['mzp']
			...
			for mA in mA_list:
                            # print (mA,mzp)
                            isfill=False
                            if (int(mzp)) >= (int(mA) + 125.  ): isfill = True
...
for reg in regions:
	...
	if reg=='SR':
		...
		for sigHist in getSignalHists(signalDoc,modelName):
			...
			part1 = top_
			part2 = getUpperPart2_RP(reg,category)
			part3 = getUpperPart3(reg)
                        part4 = getEndPart_RP(reg)
			fout.write(part1+'\n')
			fout.write('------------'+'\n')
			fout.write(part2+'\n')
			fout.write('------------'+'\n')
...
#fix for now Monika changing outdir to outdir_ and outdir_= AllMETHisto
#outdir_="AllMETHisto"
os.system ("sed -i'.bak' 's/process1/process /g' "+outdir+"/*.txt")
...
for sigHist in getSignalHists(signalDoc,modelName):
    ...
    os.system('combineCards.py sr='+outdir+'/'+srfile+' zee='+outdir+'/monoHbb_datacard_'+year+'_ZEE_'+category+'.txt  zmumu='+outdir+'/monoHbb_datacard_'+year+'_ZMUMU_'+category+'.txt topmu='+outdir+'/monoHbb_datacard_'+year+'_TOPMU_'+category+'.txt tope='+outdir+'/monoHbb_datacard_'+year+'_TOPE_'+category+'.txt >'+outdir+'/'+outfile)
